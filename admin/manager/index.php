<?php
	...
	if ( !isset( $_SESSION['username'] ) ){
		...
		// ----------------------------------------
		  	...
		  	$term_user = unserialize($row_term['username']);
		  	$term_id = $row_term['id'];
		  	$check = false;
		  	for ( $i = 0; $i < count($term_user); $i++ ){
		  		if ( $_SESSION['username'] === $term_user[$i] ){
		  			$check = true;
		  		}
		  	}
		  	if ( $check == 0 ){
		  		header('location:../lastest');
		  	}
		...
		//-----------------------------------------
			...
			while ( $row = mysqli_fetch_assoc( $query_voca ) ) {
		    	...
		    	array_push($name_upload, $row['name']);
		  	}
	}
?>
...
<body style="background: #F0F0F0">
	<div id="load-circle">
  	...
  	<nav class="container-fuild bg-white navigation-top">
	    <div class="container pt-5">
	      	...
	      	<?php 
		        $query_author = mysqli_query($conn, 'SELECT `img`, `id`, `ho`, `ten` FROM `user` WHERE `username` = "'.$term_user[0].'"');
	      	...
	      	<br>
	      	<br>
	      	<h2 class="font-weight-bold float-left"><?php echo $term_name ?>
	      		 
	      	</h2>
	      	<h2>
	      		<?php 
	      			if ( $_SESSION['username'] === $term_user[0] ){
	      				echo '
							<i class="fas fa-trash-alt ml-4 pointer text-warning remove_term float-left" id="remove_Term" data-toggle="tooltip" data-placement="right" title="Remove This erm" onclick="removeTerm('.$term_id.', '."'".$term_name."'".')"></i>
	      				';
	      			}
	      		?>
			...
			<br>
	      	<ul class="list-inline" style="font-size: 26px;">
		    ...
		    <div class="modal-dialog modal- modal-dialog-centered modal-" role="document">
		        <div class="modal-content">
		            ...
		            <div class="modal-body">
		                ...
		                <?php
		                	for ( $i = 0; $i < count($term_user); $i++ ){
						  		if ( $i == 0 ){
						  			echo '
						  			<p><span class="font-weight-bold">Main admin</span> : '.$term_user[$i].'</p>
						  			';
						  		}
						  		else{
						  			echo '
						  			<p><span class="font-weight-bold">General amin</span> : '.$term_user[$i].'</p>
						  			';
						  		}
						  	}
		                ?>
		            ...
		            <div class="modal-footer">
		                <button type="button" class="btn btn-primary" onclick="leaveTag(<?php echo $id.','."'".$term_name."'".','. 1  ?>)">Từ chức quản trị</button>
      ...
      <a href="../../terms/question-game/?id=<?php echo $id ?>">
	          ...
	          <div class="slide-card shadow pointer" id="card-en" onclick='slideCard(0, event)'>
	             ...
	             <div class="guide-bottom">
	                <p class="text-center text-white">
	                   <span class="text-white btn-inner-icon"><i class="ni ni-ungroup" id=""></i></span>
	                   Nhấp vào thẻ để lật
	                </p>
	             </div>
	          </div>
	          <div class="slide-card shadow pointer" id="card-vn" onclick='slideCard(1, event)'>
	             ...
	             <button class="btn btn-3 btn-icon btn-white rounded" id="btn-img" data-toggle="tooltip" data-placement="bottom" title="Bật tắt chế độ hình ảnh">
	               <span class="btn-inner--icon">
	                <i class="ni ni-image" id="btn-img-i"></i>
	               </span>
	             </button>
    ...
    <nav class="container container-2 mt-4 pb-3">
        <div class="row">
            <h3>Quản lí từ vựng</h3>
        </div>
    </nav>
	
  	<div class="container mt-4 pb-5" id="add-card">
    	<button class="btn btn-success w-100 rounded-0">
        	<h5 class="text-white">Thêm thẻ</h5>
     	</button>
  	</div>

   	<button class="btn btn-success rounded-0 mt-5 float-right mr-5" id="update">Update</button>
