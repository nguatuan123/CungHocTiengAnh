...
<header class="navbar navbar-expand-lg navbar-dark bg-white shadow">
	...
	<nav class="content-cop float-right shadow bg-white pb-5" style="background: #F0F0F0 !important">
		<div class="w-100 shadow" style="height: 8em; background: #fff">
			<h4 class="font-weight-bold float-left" style="margin-left: 10%; margin-top: 1.5em">Tìm kiếm học phần của chính bạn
				<br>
			<span style="font-size: 14px; font-weight: 100;">
				Tìm kiếm và quản lí học phần của bạn
			</span>
			</h4>			
		...
		<p class="ml-5 mt-5 badge badge-default">Manager</p>
		<hr>
		<br><br>
		<div class="col-12" id="result">
			<?php
				...
				// $query = mysqli_query($conn, 'SELECT * from `term-voca` where `username` = "'.$username.'" order by `id` DESC limit 15');
				// if ( mysqli_num_rows($query) === 0 ){
    //       		}
    //       		else{
		  //         	while ($row = mysqli_fetch_assoc($query)) {
		  //             	$query_author = mysqli_query($conn, 'SELECT `id` from `user` where username = "'.$username.'"');
	...
	<!-- Core -->
	  ...
	  <!-- Search controller -->
	  <script type="text/javascript" src="../../controller/admin/my-terms/search-ajax.js"></script>
	  <!-- Index Js -->
